...
- [Chirun](chirun.qmd) is a tool to convert LaTeX notes to
...
- [Quarto](quarto.qmd) is a system which converts markdown notes into
a wide variety of formats. It is a good option if you are happy to
convert your LaTeX notes into markdown format, then work with the
markdown notes (rather than the original LaTeX source) from then
on. You can automatically convert LaTeX lecture notes into markdown
format, which can be a helpful starting point, but the conversion is
not perfect and you will need to make changes to get Quarto
working. Quarto is well documented. You can include R or python code
easily.
